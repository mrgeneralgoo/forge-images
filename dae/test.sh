...
#     Everything runs inside the container: convert-rules.py writes 0600 output
#     owned by the container's root, so generate and assert in-container rather
#     than reading it back across the bind mount as the (non-root) CI user. ---
...
JSON
  cat > "$d/custom.yaml" <<YAML
payload:
  - DOMAIN,api.example.com
  - IP-CIDR,203.0.113.0/24,no-resolve
YAML
  python3 /opt/dae/convert-rules.py --manifest "$d/manifest.json" --source-dir "$d" --output "$d/out.dae"
  # "." matches the single quotes dae emits, avoiding shell-quote gymnastics.
  grep -q "domain(full: .api.example.com.) -> MY_GROUP" "$d/out.dae"
  grep -q "dip(.203.0.113.0/24.) -> MY_GROUP" "$d/out.dae"
